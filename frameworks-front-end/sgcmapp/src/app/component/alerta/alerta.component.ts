...
export class AlertaComponent implements OnInit{
  ...
  exibeAlerta(alerta: Alerta): void {
    ...
    if (elementoAlerta && elementoAlertaMensagem) {
      elementoAlertaMensagem.innerHTML = alerta.mensagem;
  ...
  ngOnInit(): void {
    ...
    this.router.events.subscribe({
      next: (evento) => {
        if (evento instanceof NavigationStart) { //sempre que essa rota iniciar a mudança para uma nova rota ele inicia esse evento
          this.fechaAlerta();
        }
      }
    })
